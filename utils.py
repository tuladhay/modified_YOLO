...
def decode_netout(netout, obj_threshold, nms_threshold, anchors, nb_class):
    ...
    netout[..., 5:(5+nb_class)] = netout[..., 4][..., np.newaxis] * softmax(netout[..., 5:(5+nb_class)])
    netout[..., 5:(5+nb_class)] *= netout[..., 5:(5+nb_class)] > obj_threshold
    
    for row in range(grid_h):
        for col in range(grid_w):
            for b in range(nb_box):
                # from 4th element onwards are confidence and class classes
                classes = netout[row,col,b,5:(5+nb_class)]
                ...
                if classes.any():
                    ...
                    rpy = netout[row, col, b, 5:]  # TODO: where to send this?
